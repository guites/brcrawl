...
function get_dir() {
    local dir_name=$(cat /dev/urandom | tr -cd 'a-f0-9' | head -c 16)
    local dir_path="$1/runs/$dir_name"
    if [[ -d "$dir_path" ]]; then
        echo $(get_dir "$1")
    else
        echo $dir_path
...
cd "$BACKEND_DIR"
uv run flask crawl-feeds --limit 25 --output "$DIR_PATH/seeds.jsonl" --mark-crawled
...
uv run flask known-domains --output "$DIR_PATH/blocklist.txt"
...
uv run flask import-blocklist "$DIR_PATH/no_rss.txt"

# Import feeds that are not in portuguese as "blocked" to the database
uv run flask import-feeds "$DIR_PATH/lang_detect_other.jsonl" --feed-status="blocked" --output "$DIR_PATH/import_feeds_lang_detect_other.json" --descr="lang_detect_other"

# Import feeds that are **not** personal_blogs as "blocked" to the database
uv run flask import-feeds "$DIR_PATH/llm_classifier_false.jsonl" --feed-status="blocked" --output "$DIR_PATH/import_feeds_llm_classifier_false.json" --descr="llm_classifier_false"

# Import feeds considered personal_blogs as "crawled" to the database
uv run flask import-feeds "$DIR_PATH/llm_classifier_true.jsonl" --feed-status="crawled" --output "$DIR_PATH/import_feeds_llm_classifier_true.json"