...
def get_feed_by_domain(domain):
    return query_db(
        "SELECT f.id as id, domain, feed_url, status_id, fs.name as feed_status, created_at FROM feeds f INNER JOIN feed_status fs ON f.status_id = fs.id WHERE domain = ?",
        args=[domain],
        one=True,
    )
...
def get_feed_by_url(feed_url):
    return query_db(
        "SELECT f.id as id, domain, feed_url, fs.name as feed_status, created_at FROM feeds f INNER JOIN feed_status fs ON f.status_id = fs.id WHERE f.feed_url = ?",
        [feed_url],
        one=True,
    )
...
def get_feed_by_id(feed_id):
    return query_db(
        "SELECT f.id as id, domain, feed_url, fs.name as feed_status, created_at FROM feeds f INNER JOIN feed_status fs ON f.status_id = fs.id WHERE f.id = ?",
        [feed_id],
        one=True,
    )
...
def get_oldest_crawled_feed():
    return query_db(
        "SELECT f.id as id, domain, feed_url, fs.name as feed_status, created_at FROM feeds f INNER JOIN feed_status fs ON f.status_id = fs.id WHERE f.status_id = 2 ORDER BY created_at ASC",
        one=True,
    )
...
def get_blocked_feeds_description():
    return query_db(
        "SELECT f.domain, fs.name, fsh1.descr FROM feeds f INNER JOIN feed_status fs ON f.status_id = fs.id LEFT JOIN feed_status_history fsh1 ON (f.id = fsh1.feed_id ) LEFT OUTER JOIN feed_status_history fsh2 ON (f.id = fsh2.feed_id AND fsh1.created_at < fsh2.created_at) WHERE f.status_id = 4 ORDER BY fsh1.created_at DESC"
    )
...
def get_report(feed_id, hash_id):
    return query_db(
        "SELECT hash_id FROM reports WHERE feed_id = ? AND hash_id = ?",
        args=[feed_id, hash_id],
        one=True,
    )
...
def insert_report(feed_id, hash_id):
    con = get_db()
    con.execute(
        "INSERT INTO reports (feed_id, hash_id) VALUES (?, ?)", [feed_id, hash_id]
    )
    con.commit()
...
def delete_report(feed_id, hash_id):
    con = get_db()
    con.execute(
        "DELETE FROM reports WHERE feed_id = ? AND hash_id = ?", [feed_id, hash_id]
    )
    con.commit()
...
def insert_feed_history(feed_id, status_id, desc):
    con = get_db()
    con.execute(
        "INSERT INTO feed_status_history (feed_id, status_id, descr) VALUES (?, ?, ?)",
        [feed_id, status_id, desc],
    )
    con.commit()
...
def get_feeds_for_processing(num_feeds, last_process_interval):
    ...
    return query_db(
        "SELECT * FROM feeds WHERE (last_checked_at IS NULL OR last_checked_at <= ?) AND status_id IN (1, 2) AND processing_status_id = 1 ORDER BY COALESCE(last_checked_at, '1970-01-01 00:00:00') ASC LIMIT ?",
        [cutoff, num_feeds],
    )
...
def mark_feed_checked(feed_id):
    con = get_db()
    con.execute(
        "UPDATE feeds SET last_checked_at = DATETIME('now') WHERE id = ?", [feed_id]
    )
    con.commit()
...
def get_id_from_guid(feed_id, feed_item_guid):
    res = query_db(
        "SELECT id FROM feed_items WHERE feed_id = ? AND guid = ? ORDER BY id DESC LIMIT 1",
